...
var mysql = require('@vlasky/mysql2');
...
LiveMysql.prototype.zongjiManager = function(dsn, options, onBinlog) {
  ...
  newInst.once('error', function(reason) {
    ...
    self.zongji.stop();
    self.zongji.removeListener('binlog', onBinlog);
    self.zongji.removeListener('ready', onReady);
    ...
    setTimeout(function() {
      ...
      if(!('child' in self.zongji)) {
        ...
        self.zongji.child = self.zongjiManager(dsn, Object.assign({}, options, resumeoptions), onBinlog);
