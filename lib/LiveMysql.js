...
function LiveMysql(settings, binlogSettings) {
  ...
  if (typeof binlogSettings === 'function') {
    console.log(
      '\nDEPRECATION WARNING: Callback on LiveMysql constructor deprecated! \n' +
  ...
  if (typeof binlogSettings !== 'object') {
    console.log('\nDEPRECATION WARNING: binlog settings must now be passed separately as second argument to LiveMysql(). We will take care of this for now, but please adjust your legacy code to prevent things breaking in future.\n');

    binlogSettings = {};
    const prevBinlogSettingsList = ['serverId', 'minInterval', 'checkConditionWhenQueued'];
